...
class _QuizHintHomeState extends State<QuizHintHome> {
  ...
  String purchaseKey(PurchaseDetails x) {
    // For Google Play consumables, serverVerificationData contains the
    // purchase token and is the safest unique key for repeated purchases.
    final serverToken = x.verificationData.serverVerificationData.trim();
    if (serverToken.isNotEmpty) {
      return '${x.productID}:$serverToken';
    }

    // Fallbacks for platforms/builds where the server token is unavailable.
    final localData = x.verificationData.localVerificationData.trim();
    if (localData.isNotEmpty) {
      return '${x.productID}:$localData';
    }

    final purchaseId = x.purchaseID?.trim();
    if (purchaseId != null && purchaseId.isNotEmpty) {
      return '${x.productID}:$purchaseId';
    }

    return '${x.productID}:${x.transactionDate ?? ''}';
  }
    ...
    for (final x in list) {
      ...
      if (x.status == PurchaseStatus.purchased ||
        ...
        if (p == null) {
          if (mounted) {
            setState(
              () => storeMessage = 'Purchase received, but the credit pack was not found.',
            );
          }
        } else {
          final key = purchaseKey(x);

          if (!processed.contains(key)) {
            // Update the visible balance immediately.
            if (mounted) {
              setState(() {
                credits += p.credits;
            ...
            // Persist the new balance before marking the purchase as processed.
            await saveCredits();

            processed.add(key);
            await saveProcessed();
          } else if (mounted) {
            setState(() => storeMessage = 'Purchase already processed.');
          }
        }
      }
